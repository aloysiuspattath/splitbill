...
export function generateBillPdf(bill: Bill, result: CalculatedBillResult): void {
  ...
  doc.text('SplitBill Summary', 14, 18);

  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text('Private, Local Bill Splitter', 14, 25);
  ...
  const peopleRows = result.personShares.map(p => [
    p.personName,
    formatMoney(p.itemsSharePaise, currency),
    p.taxSharePaise > 0 ? formatMoney(p.taxSharePaise, currency) : '-',
    p.discountSharePaise > 0 ? `-${formatMoney(p.discountSharePaise, currency)}` : '-',
    formatMoney(p.totalPaise, currency),
  ]);
  ...
  doc.roundedRect(120, currentY, 76, 36, 3, 3, 'FD');

  doc.setFontSize(9);
  doc.setTextColor(100, 116, 139);
  doc.text('Subtotal:', 125, currentY + 8);
  doc.setTextColor(30, 41, 59);
  doc.text(formatMoney(result.subtotalPaise, currency), 190, currentY + 8, { align: 'right' });

  doc.setTextColor(100, 116, 139);
  doc.text('Total Taxes:', 125, currentY + 15);
  doc.setTextColor(30, 41, 59);
  doc.text(formatMoney(result.taxesTotalPaise, currency), 190, currentY + 15, { align: 'right' });

  if (result.discountTotalPaise > 0) {
    doc.setTextColor(100, 116, 139);
    doc.text('Total Discount:', 125, currentY + 22);
    doc.setTextColor(220, 38, 38);
    doc.text(`-${formatMoney(result.discountTotalPaise, currency)}`, 190, currentY + 22, { align: 'right' });
  }

  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.setTextColor(30, 41, 59);
  doc.text('Grand Total:', 125, currentY + 31);
  doc.setTextColor(37, 99, 235);
  doc.text(formatMoney(result.effectiveBillTotalPaise, currency), 190, currentY + 31, { align: 'right' });

  // Footer note
  doc.setFontSize(8);
  doc.setTextColor(148, 163, 184);
  doc.text('Generated with SplitBill • 100% Client-Side & Private', 14, 285);
